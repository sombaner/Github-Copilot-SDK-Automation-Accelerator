...
from copilot_client import query_models_parallel, query_model
...
async def stage3_synthesize_final(
    ...
    chairman_prompt = f"""You are the Chairman of the Gatekeeper Council. Multiple AI models have provided responses to a user's question, and then ranked each other's responses.
...
async def run_council(user_query: str) -> Dict[str, Any]:
    """
    Run the complete 3-stage council process on a query and return all results.
    ...
    Returns:
        Dict containing:
            ...
            - stage3: chairman's synthesized final response
            - metadata: label mapping and aggregate rankings
    """
...
if __name__ == "__main__":
    ...
    query = " ".join(sys.argv[1:]) if len(sys.argv) > 1 else "What are the main benefits of microservices architecture?"

    async def main():
        print(f"Running council with query: {query}\n")
        ...
        for r in result["stage2"]:
            print(f"\n--- {r['model']} ---")
            print(f"Parsed ranking: {r['parsed_ranking']}")
        ...
        print("\n" + "=" * 60)
        print("METADATA - Aggregate Rankings")
        print("=" * 60)
